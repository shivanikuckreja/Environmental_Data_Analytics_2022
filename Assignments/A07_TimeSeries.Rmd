...
# 5 NEED HELP. Doesn't change anything from 4
Days <- GaringerOzone_Wrangled %>%
  as.data.frame(dates=seq(start=2010-01-01, end=2019-12-31)) 
#Rename the column name in Days to "Date"??

# 6 # Should have final dimensions of 3652 rows and 3 columns
GaringerOzone <- left_join(GaringerOzone_Wrangled, Days)
```
...
GaringerOzone.monthly <- 
  GaringerOzone_MissingDaily %>% # is this the right dataframe? 
...
GaringerOzone.daily.ts <- 
  ts(GaringerOzone.clean$Daily.Max.8.hour.Ozone.Concentration, #clean
     start=c(2010-01-01), end=(2019-12-31), frequency=365)

GaringerOzone.monthly.ts <-
  ts(GaringerOzone.clean$mean_Ozone, #clean
     start=c(2010-01-01), end=(2019-12-31), frequency=12)
...
# 13
Mean_Monthly_Ozone <- 
  ggplot(GaringerOzone, aes(x=Date, y=mean_Ozone)) +
  geom_point()+
  geom_line+
  geom_smooth((method = "lm")
