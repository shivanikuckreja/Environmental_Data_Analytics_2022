...
water.system.name_revised <- webpage %>% 
  html_nodes("div+ table tr:nth-child(1) td:nth-child(2)") %>% html_text()
pwsid_revised <- webpage %>% 
  html_nodes("td tr:nth-child(1) td:nth-child(5)") %>% html_text()
ownership_revised <- webpage %>% 
  html_nodes("div+ table tr:nth-child(2) td:nth-child(4)") %>% html_text()
max.withdrawals.mgd_revised <- webpage %>% html_nodes("th~ td+ td") %>% html_text()


#name_web <- the_url %>% html_nodes(water.system.name) %>% html_text() #runs

#Error in parse_simple_selector(stream) : Expected selector, got <NUMBER '03' at 1>
#pwsid_web <- the_url %>% html_nodes(pwsid) %>% html_text()

#ownership_web <- the_url %>% html_nodes(ownership) %>% html_text() #runs

#Error in parse_simple_selector(stream) : Expected selector, got <NUMBER '36.0100' at 1>
#maxwithdrawals_web <- the_url %>% html_nodes(max.withdrawals.mgd) %>% html_text()

#Error in selectr::css_to_xpath(css, prefix = ".//") : 
 # The 'selector' argument must be a character vector
#month_web <- the_url %>% html_nodes(webpage) %>% html_text()
...
ggplot(withdrawals.2015, aes(x=Date,y=maxwithdrawals_web)) + 
  geom_point() + 
  labs(title = paste("Max Daily Withdrawals Across the Months"),
       subtitle = 2020,
      # scale_x_date(date_breaks="Month"), # code to show all month labels 
...
withdrawal by months for the years 2010 thru 2020.Add a smoothed line to the plot.
