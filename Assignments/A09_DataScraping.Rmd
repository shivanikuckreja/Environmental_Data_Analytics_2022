...
#Trial & Error to Insert Date Column
#Month.Year.Date  <- 
 # Max.withdrawals.monthly %>% 
  #Year=year(Date) %>%
...
#5
#Plot 
ggplot(the_df, aes(x=Month,y=max.withdrawals.mgd)) + #use date column as X once we set that column
  geom_point() + 
...
#6.
scrape.it <- function(pwsid){
  #Get the proper url
  the_url <- ifelse(pwsid==,
   'https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=2020',
                gsub('', the_year,'https://www.eia.gov/electricity/state/archive/',the_year,'/'))
