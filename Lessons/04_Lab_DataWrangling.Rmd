...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #will include values of 152 and 304
...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, year4 %in% c(1990:1999)) & lakename == "Tuesday Lake"

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, year4 %in% c(1990:1999)) %>%
...
#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 through 1999 only for July 
  ...
  NTL.phys.data %>%
    filter(year4 %>% 1990:1999) %>%
    filter(lakename == 'Tuesday Lake') %>%
    #mutate(month = month(sampledate)) %>%
    filter(month( == month(sampledate))
           filter(month == 7)
