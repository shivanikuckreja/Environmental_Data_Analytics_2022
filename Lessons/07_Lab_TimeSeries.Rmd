...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% 
  mutate( Date = my(paste0(Month,"-",Year))) %>% #now have your data object
...
#No missing data
#summarycheck for NAs in either column 
#right appraoch is to create a sequency of dates (you know starting and end observation) and then you join that with dataset. 
...
BRRainfall_ts <- ts(BRA_complete$Rainfall, start = c(fyear, fmonth, frequency = 12)

BRTemp_ts <- ts(BRA_complete$Temperature_C, start = c(fyear, fmonth, frequency = 12)
...
BRRainfall_ts <- stl(BRRainfall_ts, s.window = "periodic")
