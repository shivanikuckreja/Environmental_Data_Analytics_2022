...
```{r pull.and.plot.discharge.data}
#Set the URL
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=24&format=rdb'
...
scrape.it <- function(the_year){
  ...
  the_url <- ifelse(the_year==2019,
                    'https://www.eia.gov/electricity/state/',
                gsub('2019', the_year,'https://www.eia.gov/electricity/state/archive/',the_year,'/'))
...
the_dfs2 <- map(the_years, scrape.it) %>% bind.rows()
